...
const client = new SnowflakeDialect({client: SnowflakeDialect});

const clientWithNullAsDefault = new SnowflakeDialect(
  Object.assign({client: SnowflakeDialect}, {useNullAsDefault: true})
);

const clientWithCustomLoggerForTestWarnings = new SnowflakeDialect(
  Object.assign({client: SnowflakeDialect}, {
    log: {
...
function testsql(chain, checkValue, customClient?) {
  if (customClient) {
    chain.client = customClient;
  }
  const sqlAndBindings = chain.toSQL();
...
function testNativeSql(chain, checkValue, customClient?) {
  if (customClient) {
    chain.client = customClient;
  }
  const sqlAndBindings = chain.toSQL().toNative();
  verifySqlResult(checkValue, sqlAndBindings);
}

function testquery(chain, checkValue, customClient?) {
  if (customClient) {
    chain.client = customClient;
  }
  const sqlString = chain.toQuery();
...
describe('Custom identifier wrapping', () => {
  ...
  const clientWithCustomIdentifierWrapper = new SnowflakeDialect(
  ...
  it('should use custom wrapper', () => {
    testsql(
      ...
      clientWithCustomIdentifierWrapper
    );
  });
  ...
  it('basic select with mixed pure column and alias pair', () => {
    testsql(
      qb()
        .select('baz', {bar: 'foo'})
        .from('users'),
      'select "BAZ", "FOO" as "BAR" from "USERS"'
    );
  });
  ...
  it('allows alias with dots in the identifier name', () => {
    testsql(
      qb()
        ...
        .from('users'),
      'select "FOO" as "BAR.BAZ" from "USERS"'
    );
  });
        ...
        }),
{
    ...
    testquery(
      ...
      'select * from "USERS" where ("ID" = 1 or not "ID" = 3)'
    );
  });
        ...
        .whereNotIn('id', []),
{
  ...
  it('should allow a function as the first argument, for a grouped where clause', () => {
    const partial = qb()
      .table('test')
      .where('id', '=', 1);
    testsql(partial, 'select * from "TEST" where "ID" = ?');
  ...
  it('should accept a function as the "VALUE", for a sub select', () => {
    ...
    testquery(chain, `select * where "ID" = (select "ACCOUNT_ID" from "NAMES" where "NAMES"."ID" > 1 or ("NAMES"."FIRST_NAME" like 'Tim%' and "NAMES"."ID" > 10))`);
  });
  ...
  it('should not do whereNull on where("FOO", "<>", null) #76', () => {
    testquery(qb().where('foo', '<>', null), 'select * where "FOO" <> NULL');
  });

  it('should expand where("FOO", "!=") to - where id = "!="', () => {
    testquery(qb().where('foo', '!='), `select * where "FOO" = '!='`);
  });
        ...
        .orderBy('age', 'desc'),
{
  ...
  it('or having', () => {
    testsql(
      qb()
        ...
        .having('baz', '>', 5)
        .orHaving('email', '=', 10),
      'select * from "USERS" having "BAZ" > ? or "EMAIL" = ?'
    );
  });
  ...
  it('having null', () => {
    testsql(
      qb()
        ...
        .from('users')
        .havingNull('baz'),
      'select * from "USERS" having "BAZ" is null'
    );
  });
  ...
  it('having not null', () => {
    testsql(
      qb()
        ...
        .from('users')
        .havingNotNull('baz'),
      'select * from "USERS" having "BAZ" is not null'
    );
  });
  ...
  it('having exists', () => {
    testsql(
      qb()
        .select('*')
        .from('users')
        // @ts-ignore the knex type defs don't include this for some reason?
  ...
  it('where between', () => {
    testsql(
      ...
      'select * from "USERS" where "BAZ" between ? and ?'
    );
  });
  ...
  it('where not between', () => {
    testsql(
      ...
      'select * from "USERS" where "BAZ" not between ? and ?'
    );
  });
  ...
  it('where in', () => {
    testsql(
      qb()
        ...
        .from('users')
        .whereIn('baz', [5, 10, 37]),
      'select * from "USERS" where "BAZ" in (?, ?, ?)'
    );
  });
  ...
  it('where not in', () => {
    testsql(
      ...
      'select * from "USERS" where "BAZ" not in (?, ?, ?)'
    );
  });
  ...
  it('should throw warning with null call in limit', function () {
    try {
      testsql(
        ...
        clientWithCustomLoggerForTestWarnings
      );
    } catch (error) {
...
'select * from "FILES" where "PATH" = \'C:\\\\test.txt\'',
    );
  });
  ...
  it('#965 - .raw accepts Array and Non-Array bindings', () => {
    ...
    testquery(
      qb()
        ...
        .from('users')
        .where(raw('updtime = ?', date)),
`select * from "USERS" where updtime = '` + sqlUpdTime + "'"
    );
  });
        ...
        }),
{
  ...
  it('#1228 Named bindings', () => {
    ...
    const snowflakeQb = client
      .queryBuilder()
      .select('*')
      .from('users')
      .where(
        client.raw(':name: = :thisGuy or :name: = :otherGuy', namedBindings)
      )
      .toSQL();
    ...
    it('with update query passed as raw', () => {
      testquery(
        qb()
          ...
          .from('accounts'),
`with "UPDATE1" as (update "ACCOUNTS" set "NAME" = 'foo') select * from "ACCOUNTS"`,
      );
    });
    ...
    it('with delete query passed as raw', () => {
      testquery(
        qb()
          ...
          .from('accounts'),
`with "DELETE1" as (delete from "ACCOUNTS" where "ID" = 1) select * from "ACCOUNTS"`,
      );
    });
    ...
    it('with places bindings in correct order', () => {
      testquery(
        qb()
          ...
          .update({name: 'foo'})
          .where({group_id: 1}),
`with "UPDATED_GROUP" as (update "GROUP" set "GROUP_NAME" = 'bar' where "GROUP_ID" = 1) update "USER" set "NAME" = 'foo' where "GROUP_ID" = 1`,
      );
    });
  });
  ...
  it('Can call knex.select(0)', () => {
    testquery(qb().select(0), 'select 0');
  });
